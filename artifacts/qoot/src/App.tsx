...
function TrialBanner({ profile }: { profile?: Profile }) {
  const t = useT();
  if (!profile || profile.trialStatus === 'not_started') return null;
  const active = profile.trialStatus === 'active';
  return <div data-testid="trial-status-banner" className={`mb-6 flex flex-col gap-3 rounded-2xl border p-4 sm:flex-row sm:items-center sm:justify-between ${active ? 'border-primary/25 bg-primary/10' : 'border-accent/30 bg-accent/10'}`}><div><p className="text-sm font-bold text-foreground">{active ? t(`${profile.trialDaysRemaining} days left in your free trial`, `فاضلك ${profile.trialDaysRemaining} أيام في التجربة المجانية`) : t('Your Qoot Plus trial has ended', 'التجربة المجانية لـ Qoot Plus انتهت')}</p><p className="mt-1 text-xs text-muted-foreground">{active ? t('Enjoy unlimited AI scans, full AI Coach access, and kitchen memory.', 'استمتع بسكانات AI غير محدودة، وكوتش كامل، وذاكرة مطبخك.') : t('Subscribe to keep using premium AI features. Basic tracking stays available.', 'اشترك عشان تكمّل مميزات الذكاء الاصطناعي. تسجيل الأكل والتقدم يفضلوا متاحين.')}</p></div><Link href="/checkout" data-testid="link-trial-upgrade" className="qoot-button inline-flex shrink-0 items-center justify-center rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground">{active ? t('View Qoot Plus', 'شوف قوت بلس') : t('Subscribe · 99 EGP / $2', 'اشترك · 99 جنيه / 2 دولار')}</Link></div>;
}

function PremiumGate() {
  const t = useT();
  return <div data-testid="premium-expired-gate" className="qoot-page-in mx-auto max-w-2xl"><div className="qoot-card rounded-3xl p-8 text-center"><Sparkles className="mx-auto mb-4 h-10 w-10 text-primary" /><h1 className="qoot-display text-3xl">{t('Qoot Plus is ready when you are', 'قوت بلس مستنيك')}</h1><p className="mt-3 text-muted-foreground">{t('Your 7-day trial has ended. Subscribe to unlock full AI Coach and unlimited AI meal scans. Your basic tracking remains available.', 'تجربتك المجانية لمدة ٧ أيام انتهت. اشترك عشان تفتح الكوتش الكامل وسكانات الوجبات غير المحدودة. تسجيل الأكل والتقدم يفضلوا متاحين.')}</p><Link href="/checkout" data-testid="link-premium-upgrade" className="qoot-button mt-6 inline-flex rounded-xl bg-primary px-5 py-3 font-bold text-primary-foreground">{t('Subscribe to Qoot Plus', 'اشترك في قوت بلس')}</Link></div></div>;
}
...
function DashboardPage() {
  ...
  return <div className="qoot-page-in space-y-8">
    <TrialBanner profile={profileQ.data} />
...
function CoachPage() {
  ...
  if (profileQ.data && !profileQ.data.premiumAccess) return <PremiumGate />;
